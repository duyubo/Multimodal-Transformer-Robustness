...
from src.models1 import  MULTModel
...
class DynamicMULTModel(MULTModel):
    def __init__(self, origin_dimensions:list, dimension, 
        ...
        self.trans = [nn.ModuleList([self.get_network(i, j, mem=False, layers = self.layers_hybrid_attn) if j != i else None for j in range(self.modality_num)]) for i in range(self.modality_num)]
    ...
    def get_network(self, mod1, mod2, mem, layers=-1):
        if not mem:
            ...
            embed_dim_in = (self.modality_num - 1) * self.d
        ...
        for i in range(modality_len):
            ...
            if not len(self.active_modality) == self.modality_num:
                ...
                for jj in self.active_modality:
                  if jj < self.active_modality[i]:
                    active_mask.extend(list(range(jj * self.d, jj * self.d + self.d)))
                  elif jj > self.active_modality[i]:
                    active_mask.extend(list(range((jj - 1) * self.d, jj * self.d)))
        ...
        if not modality_len == self.modality_num:
            ...
            for i in self.active_modality:
              for j in self.active_modality:
                if j < i:
                  active_indexes.extend(list(range((i * (self.modality_num - 1) + j) * self.d, (i * (self.modality_num - 1) + j + 1) * self.d)))
                elif j > i:
                  active_indexes.extend(list(range((i * (self.modality_num - 1) + j - 1) * self.d, (i * (self.modality_num - 1) + j) * self.d)))
    ...
    def get_active_subnet(self, active_self_attn_layer_num, 
        ...
        trans = [nn.ModuleList([self.trans[j][i].get_active_subnet(
                                                  ...
                                                  active_mask = [None]
                                                  ) if self.trans[j][i] is not None else None 
                                for i in active_modality]
                              )
...
if __name__ == '__main__':
    ...
    modality_list = [[0, 1], [0, 2], [1, 2], [0, 1, 2]]
